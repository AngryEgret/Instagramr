...
module MiniMagick
  ...
  class Image
    ...
    # colortone helper
    # sudo convert ./input.jpg \( -clone 0 -fill '#330000' -colorize 100% \) \( -clone 0 -colorspace gray -negate \) -compose blend -define compose:args=100,0 -composite ./out.jpg
    def colortone (color, level, type = 0)
    ...
    # vignette helper
    # sudo convert \( ./out2.jpg \) \( -size 900x1350 radial-gradient:none-LavenderBlush3 -gravity center -crop 600x900+0+0 +repage \) -compose multiply -flatten ./out3.jpg
    # sudo convert \( ./out3.jpg \) \( -size 900x1350 radial-gradient:#ff9966-none -gravity center -crop 600x900+0+0 +repage \) -compose multiply -flatten ./out4.jpg
    def vignette (color_1 = 'none', color_2 = 'black', crop_factor = 1.5)
      run_command("/usr/bin/convert", 
                  "\\( #{escaped_path} \\)", 
                  "\\( -size #{self["width"]*crop_factor}x#{self["height"]*crop_factor} radial-gradient:#{color_1}-#{color_2} -gravity center -crop #{self["width"]}x#{self["height"]}+0+0 +repage \\)", 
                  "-compose multiply -flatten", 
                  escaped_path)
    end
    
    # Instagram Toaster effect
    def toaster
      colortone('#330000', 100, 0)
      
      # sudo convert ./out.jpg -modulate 150,80,100 -gamma 1.2 -contrast -contrast ./out2.jpg
      run_command("/usr/bin/convert", 
                  escaped_path,
